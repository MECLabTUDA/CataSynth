...
from lib.utils.factory import get_CATARACTS_data, get_synth_data, get_optimizer, get_scheduler
...
train_ds1, _, val_ds, val_dl = get_CATARACTS_data(args, data_conf, train_conf)
data_conf['FOLD'] = -1
args.data_path = 'results/CATARACTS/2023.02.21 15_46_53/eval/'
train_ds2, _, _, _ = get_synth_data(args, data_conf, train_conf)
train_ds = ConcatDataset([train_ds1, train_ds2])
train_dl = DataLoader(train_ds, batch_size=train_conf['BATCH_SIZE'], num_workers=train_conf['NUM_WORKERS'],
...
m = timm.create_model(model_conf['TYPE'],
                      pretrained=True,
                      num_classes=train_ds1.dataset.num_tool_classes).to(DEV)
m = torch.nn.DataParallel(m, device_ids=args.device_list) if len(args.device_list) > 1 else WrappedModel(m)
...
f1_score = F1Score(num_labels=train_ds1.dataset.num_tool_classes, task='binary').to(DEV)
auroc_score = AUROC(num_labels=train_ds1.dataset.num_tool_classes, task='binary').to(DEV)
...
for epoch in range(EPOCHS):
    ...
    f1_sum = 0.
    auroc_sum = 0.
    for i, (img, _, _, _, tool_label) in enumerate(tqdm(train_dl)):
