...
from lib.utils.factory import get_synth_data, get_optimizer, get_scheduler
...
for epoch in range(EPOCHS):
    ...
    for i, (img, _, file_name, _, tool_label) in enumerate(tqdm(train_dl)):
        ...
        for n in range(4):
            ax[n].imshow(.5 + .5 * img[n].permute(1, 2, 0).cpu())
            ax[n].axis('off')
            tool_label_names = np.array(train_ds.dataset.tool_label_names).squeeze()[tool_label[n].cpu().numpy() > 0]
            ax[n].set_title(f'{file_name[0][n]}\n{tool_label_names}')
        plt.autoscale()
        plt.tight_layout()
